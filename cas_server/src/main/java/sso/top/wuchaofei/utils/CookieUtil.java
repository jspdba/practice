...
public class CookieUtil {
    ...
    public static void deleteCookie(HttpServletResponse response,String name, String domain) {
        addCookie(response, name, "", domain, 0, false);
    }
    ...
    private static void addHttpOnlyAndSecureCookie(HttpServletResponse response, String name,String value, String domain, int maxAge) throws Exception {
        //依次取得cookie中的名称、值、最大生存时间、路径、域和是否为安全协议信息
        StringBuffer strBufferCookie = new StringBuffer();
        strBufferCookie.append(name + "=" + URLEncoder.encode(value, "UTF-8") +  ";");
        if (maxAge >= 0) {
